...
def project(network_pkl: str, target_fname: str, outdir: str, save_video: bool, seed: int):
    ...
    session = '132'
    snap = '024166'
    network_pkl = 'checkpoint/stylegan2_ffhq/{}_ffhq_{}/network-snapshot-{}.pkl'.format(model, session, snap)
    target_fname = 'celeba3.jpg'
    ...
    writer = None
    if save_video:
        writer = imageio.get_writer('{}/{}_proj.mp4'.format(outdir, model), mode='I', fps=60, codec='libx264', bitrate='16M')
    ...
    # Save results.
    dist = float(round(dist[0], 4))
    print(dist)
    print(type(dist))
    PIL.Image.fromarray(proj.images_uint8[0], 'RGB').save('{}/{}_proj_{}.png'.format(outdir, model, dist))
    np.savez('{}/{}_dlatents.npz'.format(outdir, model), dlatents=proj.dlatents)
    if writer is not None:
        writer.close()
...
def main():
    ...
    parser.add_argument('--target',      help='Target image file to project to', dest='target_fname')
    ...
    parser.add_argument('--outdir',      help='Where to save the output images',  metavar='DIR')
