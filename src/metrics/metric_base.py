...
class MetricBase:
    ...
    def run(self, network_pkl, num_gpus=1, G_kwargs=dict(is_validation=True), cur_tick=None):
        self._results = []
        ...
        with tf.Graph().as_default(), tflib.create_session().as_default(): # pylint: disable=not-context-manager
            ...
            self._evaluate(G=G, D=D, Gs=Gs, G_kwargs=G_kwargs, num_gpus=num_gpus, cur_tick=cur_tick)
