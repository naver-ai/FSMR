...
def setup_training_options(
    ...
    if mix_weight is None:
        mix_weight = 1.0
        args.mix_weight = mix_weight
    args.mix_flag = mix
    args.diffaug_flag = diffaug
    ...
    with tf.Graph().as_default(), tflib.create_session().as_default(): # pylint: disable=not-context-manager
        ...
        args.train_dataset_args_val = None
    ...
    if metrics is None:
        metrics = ['fid50k_full']
    if data_name.__contains__('afhq'):
        if data_name.__contains__('dog') or data_name.__contains__('cat') or data_name.__contains__('wild'):
            metrics = ['fid5k_full']
        else:
            metrics = ['fid15k_full']
    if data_name.__contains__('lsun'):
        metrics = ['fid200k_full']
    if data_name.__contains__('metface'):
        metrics = ['fid5k_full']
    ...
    cfg_specs = {
        ...
        'auto': dict(ref_gpus=-1, kimg=25000, mb=-1, mbstd=-1, fmaps=-1, lrate=-1, gamma=-1, ema=-1, ramp=None, map=8),
        'gpu4': dict(ref_gpus=4, kimg=25000, mb=32, mbstd=8, fmaps=0.5, lrate=0.0025, gamma=1, ema=20, ramp=None, map=8), # paper256 for 4gpu
        'gpu4h': dict(ref_gpus=4, kimg=25000, mb=16, mbstd=4, fmaps=1, lrate=0.002, gamma=2, ema=10, ramp=None, map=8), # target_resolution > 256x256 for 4gpu
        ...
        'cifar':         dict(ref_gpus=2,  kimg=50000, mb=64, mbstd=32, fmaps=0.5, lrate=0.0025, gamma=0.01, ema=500, ramp=0.05, map=2),
    ...
    args.D_args.minibatch_size = args.minibatch_gpu
...
def run_training(outdir, seed, dry_run, **hyperparam_options):
    ...
    print(f'Mix Flag:    {training_options.mix_flag}')
    print(f'Mix Weight:    {training_options.mix_weight}')
    print(f'DiffAug Flag:    {training_options.diffaug_flag}')
    print()
...
def main():
    ...
    group.add_argument('--mix', help='Discriminator feature stat mix (default: false)', type=_str_to_bool, metavar='BOOL')
    group.add_argument('--mix_weight', help='FSMR weight', type=float, metavar='FLOAT')
    group.add_argument('--diffaug', help='Differential augmentation (default: false)', type=_str_to_bool, metavar='BOOL')
    ...
    group.add_argument('--cfg',   help='Base config (default: auto)', choices=['auto', 'gpu4', 'gpu4h', 'stylegan2', 'paper256', 'paper512', 'paper1024', 'cifar', 'cifarbaseline'])
