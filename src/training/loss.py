...
def eval_D(D, aug, images, labels, mix_flag=False, diffaug_flag=False, minibatch_size=None, report=None, augment_inputs=True, val_images=None, val_labels=None, return_aux=0):
    r = dnnlib.EasyDict()
    r.images_aug = images
    r.labels_aug = labels
    ...
    if diffaug_flag:
        r.images_aug = DiffAugment(r.images_aug)
    ...
    if val_images is not None:
        r.val_scores = D.get_output_for(val_images, val_labels, **r.args)

    if mix_flag:
        shuffle_indices = np.arange(minibatch_size) 
        np.random.shuffle(shuffle_indices)
        alpha = tf.random_uniform(shape=[], minval=0, maxval=1.0, dtype=tf.float32)

        r.mix_scores = D.get_output_for(r.images_aug, r.labels_aug, mix_flag=mix_flag, shuffle_indices=shuffle_indices, alpha=alpha, **r.args)  # add
    ...
    if report is not None:
        ...
        with tf.control_dependencies(report_ops):
            ...
            if mix_flag:
                r.mix_scores = tf.identity(r.mix_scores) # add
...
def stylegan2(G, D, aug, fake_labels, real_images, real_labels, mix_flag=False, mix_weight=1.0, diffaug_flag=False, minibatch_size=None, val_images=None, val_labels=None,
    ...
    D_fake = eval_D(D, aug, G_fake.images, fake_labels, mix_flag=False, diffaug_flag=diffaug_flag, minibatch_size=minibatch_size, val_images=None, val_labels=None, report='fake')
    D_real = eval_D(D, aug, real_images, real_labels, mix_flag=mix_flag, diffaug_flag=diffaug_flag, minibatch_size=minibatch_size, val_images=val_images, val_labels=val_labels, report='real')
    ...
    if mix_flag:
        ...
        """
        with tf.name_scope('Loss_mix'): # add
            mix_real_penalty = tf.square(D_real.mix_scores - D_real.scores)
            mix_real_penalty = report_stat(aug, 'Loss/mix_penalty/real', mix_real_penalty)
            D_loss += mix_real_penalty * mix_weight
