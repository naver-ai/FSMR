...
def training_loop(
    ...
    train_dataset_args_val  = {},
    metric_dataset_args     = {},       # Options for dataset to evaluate metrics against.
    metric_dataset_args2    = {},       # Options for dataset to evaluate metrics against.
    ...
    if train_dataset_args_val is not None:
        val_set = dataset.load_dataset(**train_dataset_args_val) # add val
    ...
    if train_dataset_args_val is not None:
        grid_size, grid_reals, grid_labels = setup_snapshot_image_grid(val_set) # add
        save_image_grid(grid_reals, os.path.join(run_dir, 'reals_test.png'), drange=[0,255], grid_size=grid_size) # add
    ...
    if train_dataset_args_val is not None:
        val_set.configure(minibatch_gpu) # add
    ...
    for gpu, (G_gpu, D_gpu) in enumerate(zip(G_gpus, D_gpus)):
        with tf.name_scope(f'Train_gpu{gpu}'), tf.device(f'/gpu:{gpu}'):
            ...
            with tf.name_scope('DataFetch'):
                ...
                if train_dataset_args_val is not None:
                    real_images_val_var = tf.Variable(name='val_images', trainable=False, initial_value=tf.zeros([minibatch_gpu] + val_set.shape))
                    real_labels_val_var = tf.Variable(name='val_labels', trainable=False, initial_value=tf.zeros([minibatch_gpu, val_set.label_size]))
                    real_images_val_write, real_labels_val_write = val_set.get_minibatch_tf()
                    real_images_val_write = tflib.convert_images_from_uint8(real_images_val_write)
                    data_fetch_ops += [tf.assign(real_images_val_var, real_images_val_write)]
                    data_fetch_ops += [tf.assign(real_labels_val_var, real_labels_val_write)]
            ...
            if train_dataset_args_val is not None:
                terms = dnnlib.util.call_func_by_name(G=G_gpu, D=D_gpu, aug=aug, fake_labels=fake_labels,
                                                      real_images=real_images_var, real_labels=real_labels_var,
                                                      mix_flag=mix_flag, mix_weight=mix_weight, diffaug_flag=diffaug_flag,
                                                      minibatch_size=minibatch_gpu,
                                                      val_images=real_images_val_var, val_labels=real_labels_val_var,
                                                      **loss_args)
            else:
                terms = dnnlib.util.call_func_by_name(G=G_gpu, D=D_gpu, aug=aug, fake_labels=fake_labels, real_images=real_images_var, real_labels=real_labels_var,
                                                      mix_flag=mix_flag, mix_weight=mix_weight, diffaug_flag=diffaug_flag, minibatch_size=minibatch_gpu, **loss_args)
    ...
    metrics = []
    for i, args in enumerate(metric_arg_list):
        metric = dnnlib.util.construct_class_by_name(**args)
        if train_dataset_args_val is not None:
            if i == len(metric_arg_list) - 1:
                metric.configure(dataset_args=metric_dataset_args2, run_dir=run_dir)
            else:
    ...
    while not done:
        ...
        if done or cur_tick < 0 or cur_nimg >= tick_start_nimg + kimg_per_tick * 1000:
            ...
            # end_tick = ((cur_tick * total_kimg) // cur_nimg) * 1000
            cur_percent = round(((cur_nimg/1000)/total_kimg) * 100, 1)
            ...
            if network_snapshot_ticks is not None and (done or cur_tick % network_snapshot_ticks == 0):
                ...
                if len(metrics):
                    ...
                    for metric in metrics:
                        metric.run(pkl, num_gpus=num_gpus, cur_tick=cur_tick)
