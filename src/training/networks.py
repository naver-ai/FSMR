...
def D_main(
    ...
    **_kwargs,                          # Ignore unrecognized keyword args.
    ...
    def feature_stat_mix(x):
        ...
        prob = 1.0
        if prob >= 0.5:
            mu, var = tf.nn.moments(x, axes=[2, 3], keepdims=True)
            eps = 1e-5
            apply_mu = tf.gather(mu, shuffle_indices)
            apply_var = tf.gather(var, shuffle_indices)

            # normalize
            x_norm_ins = (x - mu) / tf.sqrt(var + eps)

            # de-normalize
            x_mix = x_norm_ins * tf.sqrt(apply_var + eps) + apply_mu

            alpha_cast = tf.cast(alpha, dtype=x.dtype)
            x = alpha_cast * x + (1 - alpha_cast) * x_mix
    ...
    def fromrgb(x, y, res): # res = 2..resolution_log2
        with tf.variable_scope('FromRGB'):
            ...
            t = conv2d_layer(t, fmaps=nf(res-1), kernel=1, trainable=trainable)
            if mix_flag:
                if resolution == 32:
                    t = feature_stat_mix(t) # only cifar10
            t = adrop(t)
    ...
    def block(x, res): # res = 2..resolution_log2
        ...
        with tf.variable_scope('Conv0'):
            trainable = is_next_layer_trainable()
            x = conv2d_layer(x, fmaps=nf(res-1), kernel=3, trainable=trainable, use_spectral_norm=use_spectral_norm)
            if mix_flag:
                x = feature_stat_mix(x)
            x = adrop(x)
            x = apply_bias_act(x, act=act, clamp=conv_clamp, trainable=trainable)
        with tf.variable_scope('Conv1_down'):
            trainable = is_next_layer_trainable()
            x = conv2d_layer(x, fmaps=nf(res-2), kernel=3, down=True, resample_kernel=resample_kernel, trainable=trainable, use_spectral_norm=use_spectral_norm)
            if mix_flag:
                x = feature_stat_mix(x)
            x = adrop(x)
            x = apply_bias_act(x, act=act, clamp=conv_clamp, trainable=trainable)
    ...
    with tf.variable_scope('4x4'):
        ...
        with tf.variable_scope('Conv'):
            trainable = is_next_layer_trainable()
            x = conv2d_layer(x, fmaps=nf(1), kernel=3, trainable=trainable, use_spectral_norm=use_spectral_norm)
            if mix_flag:
                x = feature_stat_mix(x)
            x = adrop(x)
            x = apply_bias_act(x, act=act, clamp=conv_clamp, trainable=trainable)
